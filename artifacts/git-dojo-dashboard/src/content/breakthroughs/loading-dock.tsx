      ...
      <div className="flex flex-col gap-6 md:gap-8 min-h-[400px]">
        ...
        <div className="bg-[#161b22] border border-white/10 rounded-xl flex flex-col relative z-10">
          ...
          <div className="p-4 flex-1 flex flex-col gap-2 min-h-[100px]">
            ...
            {files.filter(f => f.state === 'workbench').length === 0 && (
              <div className="text-center text-muted-foreground text-sm py-4 opacity-50 italic">Empty</div>
            )}
          </div>
        </div>
        ...
        <div className="flex justify-center -my-4 relative z-0">
          <div className="bg-[#0d1117] p-2 rounded-full">
            <ArrowDown className="w-8 h-8 text-primary/40 animate-pulse" />
          </div>
        </div>

        {/* Loading Dock */}
        <div className="bg-primary/5 border border-primary/20 rounded-xl flex flex-col relative shadow-[0_0_30px_rgba(255,107,0,0.05)] z-10">
          <div className="bg-primary/10 border-b border-primary/20 p-4 text-center flex items-center justify-between">
            <div className="w-24"></div>
            <div>
              <h3 className="font-bold text-primary flex items-center justify-center gap-2">
                <TrayIcon className="w-5 h-5" /> Loading Dock
              </h3>
            ...
            <div className="w-24 flex justify-end">
              <button 
                ...
                className="bg-primary hover:bg-primary/90 text-primary-foreground disabled:opacity-50 disabled:cursor-not-allowed font-bold px-3 py-1.5 rounded-lg flex items-center gap-2 transition-all shadow-lg hover:shadow-primary/20 text-xs whitespace-nowrap"
              >
                <Truck className="w-4 h-4" /> Seal
              </button>
            </div>
          </div>
          <div className="p-4 flex-1 flex flex-col gap-2 min-h-[100px]">
            {files.filter(f => f.state === 'dock').map(f => (
              <div key={f.id} className="bg-primary/10 border border-primary/30 p-3 rounded-lg flex items-center justify-between group animate-in slide-in-from-top-4 duration-300">
            ...
            {files.filter(f => f.state === 'dock').length === 0 && (
              <div className="text-center text-primary/50 text-sm py-4 opacity-70 italic">
        ...
        <div className="flex justify-center -my-4 relative z-0">
          <div className="bg-[#0d1117] p-2 rounded-full">
            <ArrowDown className="w-8 h-8 text-emerald-500/40 animate-pulse" />
          </div>
        </div>
        ...
        <div className="bg-[#161b22] border border-white/10 rounded-xl flex flex-col relative z-10">
          <div className="bg-[#21262d] border-b border-white/10 p-4 text-center">
            <h3 className="font-bold text-emerald-400 flex items-center justify-center gap-2">
              <SealedBoxIcon className="w-5 h-5" /> The Archive
            </h3>
            <div className="text-xs text-emerald-400/60 font-mono mt-1">Repository History</div>
          </div>
          <div className="p-4 flex-1 flex flex-col gap-3 overflow-y-auto max-h-[200px]">
            {Array.from({ length: sealedCount }).map((_, i) => (
              <div key={i} className="bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-lg flex items-start gap-3 animate-in slide-in-from-top-4 duration-300">
                <SealedBoxIcon className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                <div>
                  <div className="text-sm font-bold text-emerald-400">Sealed Box #{sealedCount - i}</div>
                  <div className="text-xs text-emerald-400/70 mt-1">Contains docked files</div>
                </div>
              </div>
            ))}
            {sealedCount === 0 && (
              <div className="text-center text-muted-foreground text-sm py-4 opacity-50 italic">No commits yet</div>
            )}
          </div>
        </div>
