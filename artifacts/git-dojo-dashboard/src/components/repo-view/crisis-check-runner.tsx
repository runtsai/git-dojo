...
export function CrisisCheckRunner({
  ...
  const runCheck = useRunCrisisCheck();
  ...
  return (
    <div className="surface-card overflow-hidden flex flex-col">
      <div className="p-6 border-b border-white/5 bg-black/20 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        <div>
          <h3 className="text-lg font-bold text-foreground mb-1">Recovery Grader</h3>
          <p className="text-sm font-medium text-muted-foreground">
            Passes only when the repository is genuinely recovered.
          </p>
        </div>
        <button
          onClick={handleRun}
        ...
        >
          ...
          {runCheck.isPending ? "Grading..." : result?.passed === true ? "Grade Again" : "Grade Recovery"}
        </button>
      </div>
      ...
      {result && (
        <div className="p-6 enter-slide-up bg-background/50">
          ...
          >
            ...
            {result.passed === true ? (
              <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.2)] animate-in zoom-in-50 duration-300 ease-out motion-reduce:animate-none">
                <CheckCircle2 className="w-6 h-6 text-emerald-400" />
              </div>
            ) : result.passed === false ? (
              <div className="w-10 h-10 rounded-full bg-destructive/20 flex items-center justify-center border border-destructive/30 animate-in zoom-in-90 duration-200 motion-reduce:animate-none">
            ...
            <div className="relative z-10">
              ...
              <div className="text-xs font-medium opacity-80">
                {result.passed === true
                  ? "Badge earned. The repository is genuinely recovered."
                  : "Read the FAIL lines below — each one carries a hint."}
              </div>
            </div>
          </div>
