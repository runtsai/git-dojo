/**
 * Confirms that module 2.1's completion handler always fires
 * invalidateQueries with the progress key so the Ledger refreshes
 * without a page reload.
 *
 * The mechanism:
 *   Module2_1 fires completeModule.mutate(…)
 *     → onSuccess calls handleModule21Success(queryClient, setStep, onStepChange)
 *       → queryClient.invalidateQueries({ queryKey: getGetProgressQueryKey() })
 *       → useGetProgress() refetches automatically
 *       → completedVisualModules now includes "2.1"
 *
 * Removing or changing the invalidateQueries call in module-2-1.tsx
 * will break the relevant group below.
 */

import { describe, it, expect, vi } from "vitest";
import { getGetProgressQueryKey } from "@workspace/api-client-react";
import { handleModule21Success } from "@/content/tier2/module-2-1";

// ---------------------------------------------------------------------------
// 1. Completion handler — handleModule21Success (the real onSuccess code)
//    Removing or altering the invalidateQueries call in module-2-1.tsx
//    will break this group.
// ---------------------------------------------------------------------------

describe("handleModule21Success — invalidates the progress query", () => {
  it("calls queryClient.invalidateQueries exactly once", () => {
    const invalidateSpy = vi.fn();
    handleModule21Success({ invalidateQueries: invalidateSpy }, vi.fn());
...
describe("getGetProgressQueryKey — invalidation key stability (module 2.1)", () => {
  ...
  it("contains the progress endpoint identifier", () => {
    ...
    expect(
      ...
      "The progress query key no longer contains 'progress'. " +
        "Update the invalidateQueries call in module-2-1.tsx and this test together.",
    ).toContain("progress");
  });
});
