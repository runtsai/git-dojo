...
export const GetDueDrillsResponse = zod.object({
  ...
  "windowPasses": zod.number().describe('Number of passed runs within the rolling window (last 30 grader runs). Pair with windowFailures to compute a self-consistent pass ratio (windowPasses \/ (windowFailures + windowPasses)).\n'),
  "recovered": zod.boolean().describe('True when the learner has fully recovered: recentFailures === 0 and recentPasses > 0. Recovered entries are included in the friction list so the UI can celebrate the improvement before hiding the row.\n')
